...
ggsave(
  ...
  width = 12, height = 5 * 2.5, bg = "white"
)
...
for (.rank in rank_list) {
  ...
  .width <- 1e4 + 1000 * as.numeric(.rank == "species")
  .height <- 9e3 + 1000 * as.numeric(.rank == "species")
  png(
    file_path,
    res = 600,
    width = .width,
  ...
  pheatmap::pheatmap(
    ...
    annotation_col = phylo.prop.rank@sam_data %>%
      ...
      mutate(
        `Any alteration` = ifelse(endoscopia == "normal", "no", "yes"),
  ...
  .p <- cbind(
    ...
    theme(
      axis.text.x = element_text(
        angle = ifelse(.rank %in% c("species", "lowest"), 45, 30),
        hjust = 1
      ),
  ...
  .width <- 12 + 4 * as.numeric(.rank %in% c("species", "lowest"))
  .height <- 6 + 4 * as.numeric(.rank %in% c("species", "lowest"))
  ggsave(
